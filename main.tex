...
(2^{11})^{90}2^{-990}=2^0=1.
\]
For reference, direct evaluation gives
$\log_2(1937^{90}2^{-990})=-7.235258525707$, hence the bound is
$0.006636974627<1$.
...
The number $1937$ is also the smallest order obtained from this coarse
first-moment inequality together with the strict violation condition
$n>(k-1)^2$: the inequality is equivalent to
$n<2^{(k-1)/4}$, and checking integer $k$ shows that $k=45$ first permits
$n=(k-1)^2+1=1937$.  This is only a property of this particular bound, not a
...
is a column $c_x$: a row-side witness is impossible because the first selected
vertex in the $R$-clique already dominates all of $R$.  Since $c_x$ was not
covered by the prefix, $r_x$ was not selected (otherwise reflexivity would
cover $c_x$), and $r_x$ can be appended using $c_x$ as private witness.  Thus
every mixed sequence can be replaced by an $R$-only legal sequence of at least
the same length.  The transpose argument handles a prefix in $C$.  Conversely,
